...
#import "LFH264VideoEncoder.h"
#import "objects/DDLoggerWrapper.h"

static BOOL DDLoggerSetup = NO;
...
- (void)startLive:(LFLiveStreamInfo *)streamInfo {
    ...
    if ([self.socket isKindOfClass:LFStreamRTPTCPSocket.class]) {
        LFStreamRTPTCPSocket *rtptcpSock = (LFStreamRTPTCPSocket *)_socket;
        rtptcpSock.uid = _uid;
    }
...
- (id<LFStreamSocket>)socket {
    if (!_socket) {
        if ([_streamInfo.url isEqualToString:@"rtpudp"]) {
            _socket = [[LFStreamRTPUDPSocket alloc]initWithStream:self.streamInfo];
        }else if ([_streamInfo.url isEqualToString:@"rtptcp"]) {
            _socket = [[LFStreamRTPTCPSocket alloc]initWithStream:self.streamInfo];
        }else {
