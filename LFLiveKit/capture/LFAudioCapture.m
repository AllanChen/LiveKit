...
- (void)notifyMicrophoneOpenFailed:(OSStatus)status {
    NSLog(@"MicrophoneSource: startRunning status=%d", (int)status);
    __weak typeof(self) wSelf = self;
    dispatch_async(self.taskQueue, ^{
        [wSelf notifyMicrophoneOpenFailed:status];
    });
    usleep(1000 * 1000);
}
...
- (void)handleAudioComponentCreationFailure {
    NSLog(@"MicrophoneSource: create failed");
